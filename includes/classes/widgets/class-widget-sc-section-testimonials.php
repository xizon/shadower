...
/**
 * Widget for displaying Testimonials Carousel Section
 *
 */
if ( !class_exists( 'Shadower_Testimonials_Widget' ) ) {
	
	class Shadower_Testimonials_Widget extends WP_Widget {
	
		public function __construct() {
			$widget_ops = array(
				'classname' => 'widget_testimonials_section',
				'description' => shadower_wp_kses( __( 'Add a testimonials carousel section for your site.', 'shadower' ) ),
				'customize_selective_refresh' => true,
			);
			parent::__construct( 'testimonials_section', shadower_wp_kses( __( 'Home Section: Testimonials Carousel', 'shadower' ) ), $widget_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			global $default_data;
			$widget_demodata_testimonials = $default_data[ 'sc-home-sections' ][ 'testimonials' ];	
			
	
			$title  = apply_filters( 'widget_title', !isset( $instance['title'] ) ? ''.shadower_wp_kses( __( 'Testimonials', 'shadower' ) ).'' : $instance['title'], $instance, $this->id_base );
			...
			$content = isset( $instance['content'] ) ? shadower_wp_kses( $instance['content'] ) : $widget_demodata_testimonials;
	...
	[/uix_container]
			...
			echo "\n".'<div data-homepage-widgets-title="'.esc_attr__( 'Testimonials', 'shadower' ).'"></div>'."\n<!-- ".shadower_wp_kses( __( 'End Section', 'shadower' ) )." -->\n\n";
			...
			?>
				...
				<div class="custom-tab-group" id="custom-tab-group-<?php echo $this->id; ?>">
					...
					<div class="item">
						...
						<p>
						<a href="javascript:" class="custom-button uix_sc_testimonials-widget_btn" data-target="<?php echo esc_attr( $this->get_field_id('content') ); ?>"><i class="dashicons dashicons-plus"></i><?php echo shadower_wp_kses( __( 'Add new', 'shadower' ) ); ?></a>
						...
						<p>
						   <textarea style="display:none" id="demo-content-<?php echo esc_attr( $this->get_field_id('content') ); ?>"><?php echo $widget_demodata_testimonials; ?></textarea>
