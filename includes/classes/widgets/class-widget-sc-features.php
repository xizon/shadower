...
/**
 * Widget for displaying Features Section
 *
 */
if ( !class_exists( 'Shadower_Features_Widget' ) ) {
	
	class Shadower_Features_Widget extends WP_Widget {
	
	
		public function __construct() {
			$widget_ops = array(
				'classname' => 'widget_features_section',
				'description' => shadower_wp_kses( __( 'Add a feature section for your site.', 'shadower' ) ),
				'customize_selective_refresh' => true,
			);
			parent::__construct( 'features_section', shadower_wp_kses( __( 'Home Section: Features', 'shadower' ) ), $widget_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			global $default_data;
			$widget_demodata_features = $default_data[ 'sc-home-sections' ][ 'features' ];
			
	
			$title  = apply_filters( 'widget_title', !isset( $instance['title'] ) ? ''.shadower_wp_kses( __( 'Features', 'shadower' ) ).'' : $instance['title'], $instance, $this->id_base );
			...
			$content = isset( $instance['content'] ) ? shadower_wp_kses( $instance['content'] ) : $widget_demodata_features;
	...
	[/uix_container]
			...
			echo "\n".'<div data-homepage-widgets-title="'.esc_attr__( 'Features', 'shadower' ).'"></div>'."\n<!-- ".shadower_wp_kses( __( 'End Section', 'shadower' ) )." -->\n\n";
			...
			?>
				...
				<div class="custom-tab-group" id="custom-tab-group-<?php echo $this->id; ?>">
					...
					<div class="item">
						...
						<p>
						<a href="javascript:" class="custom-button uix_sc_form_features_col2-widget_btn" data-target="<?php echo esc_attr( $this->get_field_id('content') ); ?>"><i class="dashicons dashicons-plus"></i><?php echo shadower_wp_kses( __( 'Add 2 column', 'shadower' ) ); ?></a>
						<a href="javascript:" class="custom-button uix_sc_form_features_col3-widget_btn" data-target="<?php echo esc_attr( $this->get_field_id('content') ); ?>"><i class="dashicons dashicons-plus"></i><?php echo shadower_wp_kses( __( 'Add 3 column', 'shadower' ) ); ?></a>
						...
						<p>
						   <textarea style="display:none" id="demo-content-<?php echo esc_attr( $this->get_field_id('content') ); ?>"><?php echo $widget_demodata_features; ?></textarea>
